...
abstract class LintGradleTask extends DefaultTask {
    ...
    @Input
    @Optional
    abstract Property<GradleLintViolationAction> getConsoleOutputAction()

    @Inject
    abstract StyledTextService getTextOutput()
    ...
    LintGradleTask() {
        ...
        consoleOutputAction.convention(createConsoleOutputAction())
        group = 'lint'
    }

    @TaskAction
    void lint() {
        def violations = LintService.lint(
                getSourceFiles().files,
                getOnlyCriticalRules().get()
        ).violations.unique { v1, v2 -> v1.is(v2) ? 0 : 1 }

        def allListeners = new ArrayList<>(getListeners().get())
        allListeners.add(getConsoleOutputAction().get())
    ...
    private GradleLintViolationAction createConsoleOutputAction() {
        return new GradleLintViolationAction() {
            @Override
